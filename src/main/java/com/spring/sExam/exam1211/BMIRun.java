...
public class BMIRun {
	public static void main(String[] args) {
		...
		/*
		BMIVO p1 = (BMIVO) ctx.getBean("person1");
		p1.getService().calc(p1);
		vo.getService().printLine();
		
		BMIVO p2 = (BMIVO) ctx.getBean("person2");
		p2.getService().calc(p2);
		vo.getService().printLine();
		
		BMIVO p3 = (BMIVO) ctx.getBean("person3");
		p3.getService().calc(p3);
		vo.getService().printLine();
		*/
