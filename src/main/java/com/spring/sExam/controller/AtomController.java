...
public class AtomController {
	...
	public String a1Get(Model model, HttpServletRequest request) {
		...
		/*
		 * if(name.equals("김말숙")) model.addAttribute("gender", "여자"); 
		 * else model.addAttribute("gender", "남자");
		 */
	...
	public String a3Get(Model model, @RequestParam(name="name", defaultValue = "", required = false) String name) {
		model.addAttribute("data", "a3에서 가요.");
		model.addAttribute("name", name);
